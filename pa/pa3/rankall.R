## This function takes two arguments (an outcome name and a hospital ranking)
    ...
    ## For each state, find the hospital of the given rank
        ...
        for (i in seq_along(by_state)){
            by_state[[i]] <- by_state[[i]][order(by_state[[i]][, eval(outcome)], 
                                                 by_state[[i]][, "hospital"]), ]
            ordered[[i]]  <- c(by_state[[i]][num, "hospital"], by_state[[i]]
                               [, "state"][1])
        }
        ...
        if (num == "best") {
            ...
            ordered  <- list()
            for (i in seq_along(by_state)){
                by_state[[i]] <- by_state[[i]][order(by_state[[i]]
                                                     [, eval(outcome)], 
                                                     by_state[[i]]
                                                     [, "hospital"]), ]
